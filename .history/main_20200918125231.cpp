...
int main(int argc, char** argv) {
	...
	for (int i=0; i<af_face.nfaces(); i++) { 
    	...
    	for (int j=0; j<3; j++) { 
			...
			//垂直投影
        	screen_coords[j] = Vec3f((v.x+1.f)*height/2., (v.y+1.f)*height/2.); 
			//保存世界坐标计算法向量
			world_coords[j] = v;
    	}
    	...
    	if(intensity>0) triangle(world_coords, zbuffer, image, TGAColor(intensity*255, intensity*255, intensity*255, 255)); 
	}
