...
struct vertex_normal{
	Vec3f normal;
	float all;
	vertex_normal(Vec3f n,float a=1):normal(n),all(a) {}
};
...
int main(int argc, char** argv) {
	...
	for (int i=0; i<af_face.nfaces(); i++){
		...
		int world_index[3]; 
		for (int j=0; j<3; j++){
			world_index[j] = face[j];
			world_coords[j] = af_face.vert(world_index[j]);
		}
		...
		for (int j=0; j<3; j++){
			...
			if(it != vertices_normal.end()){
				it->second.normal=(it->second.normal*it->second.all+n)*(1/(it->second.all+1));
				it->second.all+=1;
			}
			else{
				vertices_normal.insert(std::pair<int,vertex_normal>(world_index[j],vertex_normal(n)));
			}
		}
	}
	for (int i=0; i<af_face.nfaces(); i++) { 
    	...
    	if(intensity>0) triangle(screen_coords, texture_coords, zbuffer, image, texture, light_dir, 
