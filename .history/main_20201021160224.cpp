...
int main(int argc, char** argv) {
	...
	float shadowbuffer[width*height]={0.f};
	...
	for (int i=0; i<af_face.nfaces(); i++) {
		...
		for (int j=0; j<3; j++) { 
			...
			Matrix pv = viewport(0,0,height,height)* (lookat(light_dir*(-1),origin_point,up) * Matrix(v));
	...
	for (int i=0; i<af_face.nfaces(); i++) { 
    	...
    	for (int j=0; j<3; j++) { 
			...
			//先透视再放大
			//变换
			Matrix pv = viewport(0,0,height,height) * my_perspective(2.4f, std::FLOAT_MAX, -3.f) * (lookat(eye,center,up) * Matrix(v));
			Matrix sv = viewport(0,0,height,height) * (lookat(light_dir*(-1),origin_point,up) * Matrix(v));
